            ...
            <form action="{{ route('veiculos.gastos.store') }}" method="POST" id="formCadastrarMulta">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2" name="veiculo_id" required>
                            @foreach($veiculos as $v) <option value="{{ $v->id }}">{{ $v->placa }} - {{ $v->modelo }}</option> @endforeach
                        </select>
                    </div>
                    <div class="row">
...
<script>
    ...
    $(document).ready(function() {
        $('#modalCadastrarMulta .select2').select2({
            dropdownParent: $('#modalCadastrarMulta')
        });
