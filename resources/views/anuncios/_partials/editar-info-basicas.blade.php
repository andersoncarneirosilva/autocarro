<div class="modal fade" id="modalEditarInfoBasica" tabindex="-1" aria-labelledby="modalEditarInfoBasicaLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('anuncios.updateInfoBasica', $veiculo->id) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="modal-body">
        <div class="row">
            <div class="col-md-12 mb-3">
                <label for="estado_veiculo" class="form-label text-muted">Estado do Veículo</label>
                <select class="form-select" name="estado" id="estado" required>
                    <option value="" disabled {{ !isset($veiculo->estado) ? 'selected' : '' }}>Selecione o estado</option>
                    <option value="Novo" {{ $veiculo->estado == 'Novo' ? 'selected' : '' }}>Novo (0km)</option>
                    <option value="Semi-novo" {{ $veiculo->estado == 'Semi-novo' ? 'selected' : '' }}>Semi-novo</option>
                    <option value="Usado" {{ $veiculo->estado == 'Usado' ? 'selected' : '' }}>Usado</option>
                </select>
            </div>
