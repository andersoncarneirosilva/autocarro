...
class DocumentoController extends Controller
{
    ...
    public function gerarProcuracao(Request $request, $veiculoId)
    {
        ...
        // Substituições de Tags do Veículo e Cliente
        $substituicoes = [
    ...
    private function gerarPdfAtpve(Request $request, $veiculoId)
    {
        ...
        $veiculo = Veiculo::findOrFail($veiculoId);
        $cliente = Cliente::findOrFail($request->cliente_id); 
        $modelo = ModeloAtpve::where('user_id', $userId)->firstOrFail();
        
        // Proteção: busca o vendedor (outorgado) ou cria um objeto vazio para evitar erro de null
        $vendedor = Outorgado::where('user_id', $userId)->first();
        $hoje = \Carbon\Carbon::now();
        $html = $modelo->conteudo;

        $substituicoes = [
            // Vendedor (Tags de Outorgado)
            ...
            '{MARCA_MODELO}'     => strtoupper($veiculo->marca . ' / ' . $veiculo->modelo),
            '{COR}'              => strtoupper($veiculo->cor),
            '{VALOR_VENDA}'      => 'R$ ' . $request->valor_venda,

            // Adquirente (Tags de Cliente)
            '{NOME_CLIENTE}'        => strtoupper($cliente->nome),
            '{CPF_CLIENTE}'         => $cliente->cpf,
            ...
            '{ESTADO_CLIENTE}'      => strtoupper($cliente->estado), // ou $cliente->uf
